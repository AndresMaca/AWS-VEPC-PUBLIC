...
//string g_trafmon_ip_addr2 = RAN2;
//string g_trafmon_ip_addr3 = RAN3;

string g_mme_ip_addr = MME1;

string g_hss_ip_addr = HSS;
string g_sgw_s11_ip_addr = SGWLB;
string g_sgw_s1_ip_addr = SGWLB;
string g_sgw_s5_ip_addr = SGWLB;
string g_pgw_s5_ip_addr = PGWLB;
string g_mme_rep_ip1 = MME2;
string g_mme_rep_ip2 = MME3;
...
void Mme::handle_initial_attach(int conn_fd, Packet pkt, SctpClient &hss_client, int worker_id) {
	...
	if(UE_BINDING == STATE_LESS) push_context(guti,local_ue_ctx,local_ue_ctx.mme_s1ap_ue_id,worker_id); //push to store
...
bool Mme::pull_context(Packet pkt,int worker_id){
...
bool Mme::handle_autn(int conn_fd, Packet pkt, int worker_id) {
	...
	if(UE_BINDING == STATE_LESS){
		...
		if(pull_context(pkt,worker_id)==false)
				return false;
	...
	if (guti == 0) {
		TRACE(cout << "mme_handleautn:" << " zero guti " << pkt.s1ap_hdr.mme_s1ap_ue_id << " " << pkt.len << ": " << guti << endl;)
										return false;		g_utils.handle_type1_error(-1, "Zero guti: mme_handleautn");
	}
...
void Mme::handle_security_mode_cmd(int conn_fd, Packet pkt, int worker_id) {
	...
	if (guti == 0) {
		TRACE(cout << "mme_handlesecuritymodecmd:" << " zero guti " << pkt.s1ap_hdr.mme_s1ap_ue_id << " " << pkt.len << ": " << guti << endl;		)
										return ;		g_utils.handle_type1_error(-1, "Zero guti: mme_handlesecuritymodecmd");
	}	
...
bool Mme::handle_security_mode_complete(int conn_fd, Packet pkt, int worker_id) {
	...
	if (guti == 0) {
		TRACE(cout << "mme_handlesecuritymodecomplete:" << " zero guti " << pkt.s1ap_hdr.mme_s1ap_ue_id << " " << pkt.len << ": " << guti << endl;		)
										return false;		g_utils.handle_type1_error(-1, "Zero guti: mme_handlesecuritymodecomplete");
	}		
...
void Mme::handle_location_update(Packet pkt, SctpClient &hss_client, int worker_id) {
	...
	if (guti == 0) {
		TRACE(cout << "mme_handlelocationupdate:" << " zero guti " << pkt.s1ap_hdr.mme_s1ap_ue_id << " " << pkt.len << ": " << guti << endl;		)
								return ;				g_utils.handle_type1_error(-1, "Zero guti: mme_handlelocationupdate");
	}
	UeContext local_ue_ctx;
	g_sync.mlock(uectx_mux);
	local_ue_ctx = ue_ctx[guti];
...
void Mme::handle_create_session(int conn_fd, Packet pkt, vector<UdpClient>& sgw_clients,UdpClient& sgw_client, int worker_id) {
	int err_code = 0;
	...
	if (guti == 0) {
		TRACE(cout << "mme_handlecreatesession:" << " zero guti " << pkt.s1ap_hdr.mme_s1ap_ue_id << " " << pkt.len << ": " << guti << endl;		)
								return ;				g_utils.handle_type1_error(-1, "Zero guti: mme_handlecreatesession");
	}		
	eps_bearer_id = 5;
	set_pgw_info(guti);
	UeContext local_ue_ctx;
	...
	{
		int index = getIndex(s11_cteid_mme,sgw_clients.size());
		g_sync.mlock(thread_mux[index]);
		err_code = sgw_clients[index].snd(pkt);
	}
...

	{
		...
		int index = getIndex(s11_cteid_mme,sgw_clients.size());
		err_code = sgw_clients[index].rcv(pkt);
		g_sync.munlock(thread_mux[index]);
	}
...
void Mme::handle_attach_complete(Packet pkt, int worker_id) {
	...
	if (guti == 0) {
		TRACE(cout << "mme_handleattachcomplete:" << " zero guti " << pkt.s1ap_hdr.mme_s1ap_ue_id << " " << pkt.len << ": " << guti << endl;		)
							return ;					g_utils.handle_type1_error(-1, "Zero guti: mme_handleattachcomplete");
	}
...
void Mme::handle_modify_bearer(int conn_fd,Packet pkt, vector<UdpClient>& sgw_clients,UdpClient& sgw_client, int worker_id) {
	int err_code = 0;
	uint64_t guti;
	uint32_t s1_uteid_dl;
	uint32_t s11_cteid_sgw;
	...
	if (guti == 0) {
		TRACE(cout << "mme_handlemodifybearer:" << " zero guti " << pkt.s1ap_hdr.mme_s1ap_ue_id << " " << pkt.len << ": " << guti << endl;		)
							return ;					g_utils.handle_type1_error(-1, "Zero guti: mme_handlemodifybearer");
	}	
	...
	/* uncomment when using multiple ran
	
		if(local_ue_ctx.imsi%3==0)
			pkt.append_item(g_trafmon_ip_addr); //even UE to ran1
		else if (local_ue_ctx.imsi%3==1)
			pkt.append_item(g_trafmon_ip_addr2); //odd UE to ran2
		else
			pkt.append_item(g_trafmon_ip_addr3); //odd UE to ran2
	
	*/
	
	
	/* comment when using multiple ran */
	...
	pkt.prepend_gtp_hdr(2, 2, pkt.len, s11_cteid_sgw);
	{
		...
		int index = getIndex(local_ue_ctx.s11_cteid_mme,sgw_clients.size());
		g_sync.mlock(thread_mux[index]);
		err_code = sgw_clients[index].snd(pkt);
	...
	if (res == false || err_code !=0) {
		...
		if(REP) {
			Mme_state state_data;
			state_data = Mme_state(local_ue_ctx,guti);
			send_replication(1,state_data, local_ue_ctx.mme_s1ap_ue_id,guti,worker_id);
...
void Mme::handle_detach(int conn_fd, Packet pkt, vector<UdpClient>& sgw_clients,UdpClient& sgw_client, int worker_id) {
	int err_code = 0;
	...
	if (guti == 0) {
		TRACE(cout << "mme_handledetach:" << " zero guti " << pkt.s1ap_hdr.mme_s1ap_ue_id << " " << pkt.len << ": " << guti << endl;)
								return ;				g_utils.handle_type1_error(-1, "Zero guti: mme_handledetach");
	}
	...
	if( (UE_BINDING == SESSION_BASED) || (UE_BINDING==STATE_LESS) )erase_context(guti,worker_id);
	if(REP){
		Mme_state state_data;
		send_replication(0,state_data,pkt.s1ap_hdr.mme_s1ap_ue_id,guti,worker_id);
	}
...
repType = 0  erase
	*/
void Mme::send_replication(int repType,Mme_state ctx,uint32_t mme_s1ap_ue_id,uint64_t guti,int worker_id){
	...
	mme_rep_clients1[worker_id].snd(pkt);
	mme_rep_clients2[worker_id].snd(pkt);


}

void Mme::receive_replication(Packet pkt,int worker_id){
	...
	if(repType){
		...
		Mme_state mme_state = toBoostObject<Mme_state>(boostedState);
